...
<template>
  ...
  >
    <button
      @click="isDark = !isDark"
      class="absolute top-4 right-4 bg-gray-200 dark:bg-gray-800 p-2 rounded-full"
    >
      {{ isDark ? '🌙' : '☀️' }}
    </button>
    ...
    <div class="grid grid-cols-2 gap-4 sm:gap-6 max-w-md w-full">
      ...
      <div class="flag-inner bg-white dark:bg-gray-800 p-4 rounded-xl shhadow-md transition transform hover:scale-105">
        <img
          :src="country.flag"
          :alt="country.name"
